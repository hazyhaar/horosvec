...
// sync.Pool.Get may return state from previous query. reset() reallocates but doesn't validate prior bitset contents.
func acquireSearchState(maxNodes int64, beamWidth int, dim int) *searchState {
	s := searchPool.Get().(*searchState) // pool type is guaranteed by Put
	s.reset(maxNodes, beamWidth, dim)
	return s
}

// releaseSearchState returns a searchState to the pool.
// Pool.Put — caller MUST defer this. If panic before defer, state leaked from pool = fresh allocations on subsequent acquires.
